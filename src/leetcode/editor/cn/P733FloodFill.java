...
public class P733FloodFill {
    ...
    class Solution {
        ...
        public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
            row = image.length;
            col = image[0].length;
            isReFilled = new boolean[row][col];
        ...
        private void helper(int[][] image, int sr, int sc, int oldColor, int newColor) {
            if (!isReFilled[sr][sc]) {
                ...
                isReFilled[sr][sc] = true;
                if (sr - 1 >= 0 && !isReFilled[sr - 1][sc] && image[sr - 1][sc] == oldColor)
                    helper(image, sr - 1, sc, oldColor, newColor);
                if (sr + 1 < row && !isReFilled[sr + 1][sc] && image[sr + 1][sc] == oldColor)
                    helper(image, sr + 1, sc, oldColor, newColor);
                if (sc - 1 >= 0 && !isReFilled[sr][sc - 1] && image[sr][sc - 1] == oldColor)
                    helper(image, sr, sc - 1, oldColor, newColor);
                if (sc + 1 < col && !isReFilled[sr][sc + 1] && image[sr][sc + 1] == oldColor)
                    helper(image, sr, sc + 1, oldColor, newColor);
            }
        }
    }
