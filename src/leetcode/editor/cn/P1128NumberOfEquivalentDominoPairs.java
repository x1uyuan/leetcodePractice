//给你一个由一些多米诺骨牌组成的列表 dominoes。 
//
// 如果其中某一张多米诺骨牌可以通过旋转 0 度或 180 度得到另一张多米诺骨牌，我们就认为这两张牌是等价的。 
//
// 形式上，dominoes[i] = [a, b] 和 dominoes[j] = [c, d] 等价的前提是 a==c 且 b==d，或是 a==d 且 
//b==c。 
//
// 在 0 <= i < j < dominoes.length 的前提下，找出满足 dominoes[i] 和 dominoes[j] 等价的骨牌对 (i,
// j) 的数量。 
//
// 
//
// 示例： 
//
// 输入：dominoes = [[1,2],[2,1],[3,4],[5,6]]
//输出：1
// 
//
// 
//
// 提示： 
//
// 
// 1 <= dominoes.length <= 40000 
// 1 <= dominoes[i][j] <= 9 
// 
// Related Topics 数组 
// 👍 31 👎 0


package leetcode.editor.cn;

/**
 * Java：等价多米诺骨牌对的数量
 * date：2020-09-24 16:23:51
 **/
public class P1128NumberOfEquivalentDominoPairs {
    public static void main(String[] args) {
        Solution solution = new P1128NumberOfEquivalentDominoPairs().new Solution();
        // TO TEST
        System.out.println(solution.numEquivDominoPairs(new int[][]{{1, 1}, {2, 2}, {1, 1}, {1, 2}, {1, 2}, {1, 1}}));
    }

    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public int numEquivDominoPairs(int[][] dominoes) {
            int res = 0;
            int[][] count = new int[9][9];
            for (int i = 0; i < dominoes.length; i++) {
                int small = Math.min(dominoes[i][0], dominoes[i][1]);
                int big = Math.max(dominoes[i][0], dominoes[i][1]);
                count[small - 1][big - 1] = count[small - 1][big - 1] + 1;
            }
            for (int i = 0; i < 9; i++) {
                for (int j = i; j < 9; j++) {
                    res += count[i][j] * (count[i][j] - 1) / 2;
                }
            }
            return res;
        }
    }
//leetcode submit region end(Prohibit modification and deletion)

}
